...
const tipoSinGrupo = (tipo: string): boolean => {
  ...
  return t.includes('semi elaborado') || t.includes('producto terminado')
}
...
export default function ProductosPage() {
  ...
  const filtered = records.filter(r => {
    ...
    const target = norm(r.codigo) + ' ' + norm(r.descripcion)
  ...
  const printBarcodes = () => {
    ...
    if (conBarcode.length === 0) { alert(tAl('noProductosCodigoBarra')); return }
    ...
    const items = conBarcode.map((r, idx) => `
      <div class="item">
        ...
        ${showQR ? `<canvas class="qr" id="qr${idx}" data-text="${r.codigo_barra}|${r.descripcion}" width="120" height="120"></canvas>` : ''}
    ...
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .meta{font-size:9px;color:#1a56db;font-weight:600;margin-top:2px}
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {tab === 'registros' && permisos.editar && (
          <div className="flex gap-3 flex-wrap">
            ...
            <button onClick={() => { setForm(initForm(records, tipoInvFiltro || '')); setIsFormOpen(true) }} className="px-5 py-2.5 rounded-xl font-medium text-white" style={{ background: 'rgba(59,130,246,1)', border: '1px solid rgba(37,99,235,1)' }}>
              {tBtn('newProduct')}
            </button>
          </div>
        )}
          ...
          {isFormOpen && (
            <div className="mb-8 rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }}>
              ...
              <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {/* Campos texto libre */}
                {[
                  { label: 'Descripción *', key: 'descripcion', placeholder: 'Nombre del producto', span: true },
                ].map(({ label, key, placeholder, span }: { label: string; key: string; placeholder: string; span?: boolean }) => (
                  <div key={key} className={span ? 'lg:col-span-2' : ''}>
                    <label className="block text-xl font-extrabold text-white mb-1">{label}</label>
                    <input required={label.includes('*')} value={String((form as Record<string, unknown>)[key])} onChange={e => setForm({ ...form, [key]: e.target.value })}
                      className="w-full rounded-xl px-4 py-2.5 text-white outline-none"
                      style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.15)' }}
                      placeholder={placeholder} />
                  </div>
                ))}
                {/* Tipo Inventario — tabla referencia */}
                <div>
                  <label className="block text-xl font-extrabold text-white mb-1">Tipo Inventario</label>
                  <select value={form.tipo_inventario} onChange={e => {
                ...
                {/* Último Proveedor — tabla referencia */}
                <div>
                ...
                {/* Campos numéricos */}
                {['ult_costo', 'costo_promedio', 'precio_unitario', 'maximo', 'minimo', 'existencia'].map(key => (
                  <div key={key}>
          ...
          <div className="rounded-2xl overflow-x-auto" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <table className="w-full text-xs text-left table-fixed">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={11} className="px-6 py-12 text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>{tE('noProductos')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === 'especificos' && (
        <div className="glass-card p-6 md:p-8">
            ...
            return (
              <div>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-lg font-bold text-white">{tRpt('inventariosCategoria')}</h3>
                </div>
                ...
                <div className="flex items-end gap-4 mb-6 flex-wrap">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      doc.text('Reporte de Inventarios por Categoría', logoOff, 13)
                      doc.setFontSize(9)
                ...
                <div className="mt-4">
                  <span className="text-xs text-white/40">{reporteFecha ? `Fecha: ${reporteFecha}` : ''} — {categoriasOrdenadas.length} categorías</span>
                </div>
              </div>
            )
          })()}
          ...
          {reporteActivo === 'inventario-valorizado' && (() => {
            const totalGeneral = records.reduce((sum, r) => sum + (r.ult_costo * r.existencia), 0)
            return (
              <div>
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-lg font-bold text-white">Inventario Valorizado</h3>
                </div>
                ...
                <div className="flex items-end gap-4 mb-6">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      doc.setFontSize(16)
                      doc.setFont('helvetica', 'bold')
                      doc.text('Inventario Valorizado', logoOff, 13)
                      doc.setFontSize(9)
                      doc.setFont('helvetica', 'normal')
                      doc.setTextColor(180, 180, 210)
                      doc.text(`Fecha: ${reporteFecha || new Date().toLocaleDateString('es-CO', { timeZone: 'America/Bogota' })}`, logoOff, 21)
                      ...
                      doc.text(`Total productos: ${records.length}`, m, y)
                ...
                <div className="mt-4 flex justify-between items-center">
                  <span className="text-xs text-white/40">
                    {reporteFecha ? `Fecha: ${reporteFecha}` : ''} — {records.length} productos
                  </span>
                </div>
              </div>
            )
          ...
          <div className="w-full max-w-2xl rounded-2xl p-6 max-h-[90vh] overflow-y-auto" style={{ background: 'rgba(15,23,42,0.95)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                ...
                { label: 'Usa Seriales', value: viewRecord.usa_seriales ? 'Sí' : 'No' },
                ...
                { label: 'Código de Barra', value: viewRecord.codigo_barra || '—' },
                ...
                { label: 'Ult. Documento', value: viewRecord.nro_ult_documento || '—' },
                { label: 'Tipo Ult. Movimiento', value: viewRecord.tipo_ult_movimiento || '—' },
          ...
          <div className="w-full max-w-lg rounded-2xl p-6 space-y-5" style={{ background: 'rgba(15,23,42,0.98)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select value={etqProducto} onChange={e => setEtqProducto(e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: 'rgba(12,26,61,0.9)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff' }}>
                  <option value="">{tOp('todosProductos')}</option>
                  {records.filter(r => r.situacion === 'Activo' && r.codigo_barra).map(r => (
            ...
            <div className="text-xs text-white/40">
              {(() => {
                ...
                return `${base.length} etiquetas a generar — ${tipo}`
              })()}
            </div>
