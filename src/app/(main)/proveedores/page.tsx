...
const initForm = (): Proveedor => ({
  ...
  persona_contacto: '', referencias: '', observacion_referencia: '', direccion: '', ciudad: '', pais: 'Colombia', situacion: 'Activo',
...
export default function ProveedoresPage() {
  ...
  const fields: { label: string; key: keyof Proveedor; placeholder?: string; span?: boolean }[] = [
    { label: tF('nombreRequired'), key: 'nombre', placeholder: 'Razón Social', span: true },
    { label: 'Nro. Documento *', key: 'nro_documento', placeholder: 'J-12345678-9' },
    { label: 'Correo', key: 'correo', placeholder: 'email@proveedor.com' },
    { label: 'Telf. Oficina', key: 'telf_oficina', placeholder: '0212-0000000' },
    { label: 'Móvil Oficina', key: 'movil_oficina', placeholder: '0414-0000000' },
    { label: tF('personaContacto'), key: 'persona_contacto', placeholder: 'Nombre del contacto' },
    { label: 'Dirección', key: 'direccion', placeholder: 'Av. Principal #123', span: true },
  ]
          ...
          {isFormOpen && (
            <div className="mb-8 rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }}>
              ...
              <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {/* Tipo de Identificación — tabla referencia */}
                <div>
                ...
                {fields.map(({ label, key, placeholder, span }) => {
                  const isNombre = key === 'nombre'
                  const isDoc = key === 'nro_documento'
                  const isCorreo = key === 'correo'
                  const showError = (isNombre && nombreDuplicadoLive) || (isDoc && docDuplicadoLive) || (isCorreo && correoDuplicadoLive)
                  return (
                    <div key={key} className={span ? 'lg:col-span-3' : ''}>
                      ...
                      <input
                        ...
                        style={{
                          ...
                          border: showError ? '1px solid rgba(239,68,68,0.6)' : '1px solid rgba(255,255,255,0.15)',
                        }}
                        placeholder={placeholder}
                      />
                      {isNombre && nombreDuplicadoLive && (
                        <p className="text-xs mt-1 font-medium" style={{ color: '#fff' }}>
                          ⚠ Ya existe un proveedor con este nombre
                        </p>
                      )}
                      {isDoc && docDuplicadoLive && (
                        <p className="text-xs mt-1 font-medium" style={{ color: '#fff' }}>
                          ⚠ Ya existe un proveedor con este documento
                        </p>
                      )}
                      {isCorreo && correoDuplicadoLive && (
                        <p className="text-xs mt-1 font-medium" style={{ color: '#fff' }}>
                          ⚠ Ya existe un proveedor con este correo
                        </p>
                      )}
                    </div>
                  )
                })}
                {/* Referencia Proveedor — tabla referencia */}
                <div>
                ...
                {/* Situación — al final, justo antes de los botones */}
                <div className="lg:col-span-3 mt-4 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.1)' }}>
          ...
          <div className="rounded-2xl overflow-x-auto" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <table className="w-full text-base text-left">
              <thead style={{ background: 'rgba(255,255,255,0.05)' }}>
                <tr>
                  {[tF('nombre'), tH('documento'), tH('actividad'), tH('contacto'), tF('telefonoSingle'), tF('ciudad'), tF('estado'), tTbl('actions')].map(h => (
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={8} className="px-6 py-12 text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>{tE('noProveedores')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
