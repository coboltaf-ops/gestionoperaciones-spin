...
export default function RecepcionFacturasPage() {
  ...
  const handleCantidad = (idx: number, val: string) => {
    ...
    if (val === '') {
      ...
      renglones[idx] = { ...r, cantidad_a_recibir: 0 }
      setForm({ ...form, renglones })
      return
    }
    const num = parseFloat(val)
    if (isNaN(num) || num < 0) {
      setRenglonErrors(prev => ({ ...prev, [idx]: 'Cantidad inválida' }))
      renglones[idx] = { ...r, cantidad_a_recibir: 0 }
      setForm({ ...form, renglones })
      return
    }
    if (num > pendiente) {
      // Cantidad supera el saldo pendiente → bloquear y resetear
      setRenglonErrors(prev => ({ ...prev, [idx]: 'Cantidad inválida' }))
      renglones[idx] = { ...r, cantidad_a_recibir: 0 }
      setForm({ ...form, renglones })
      return
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(renglonErrors).length > 0) { setErrorMsg('Hay cantidades inválidas en los items. Corrígelas antes de guardar.'); return }
    ...
    <div>
      <div className="flex items-start mb-8 gap-4 flex-wrap">
        ...
        {tab === 'registros' && permisos.editar && (
          <button
            onClick={() => { setForm(emptyForm(todasRecepciones.length + 1, tipoActivo || '')); setIsFormOpen(true); setErrorMsg(''); setOcPendienteMsg(''); setRenglonErrors({}) }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <PersonalSelector
                      ...
                      onChange={v => setForm({ ...form, persona_recibe: v })}
                      placeholder="Seleccione la persona que recibe…"
                      className="w-full rounded-xl px-4 py-2.5 outline-none text-white"
                      style={inputStyle}
                      required
                    />
                  </div>
                                  ...
                                  {r.completo ? (
                                    ...
                                    <div>
                                      <input
                                        ...
                                        style={{
                                          background: renglonErrors[idx] ? 'rgba(239,68,68,0.15)' : 'rgba(96,165,250,0.15)',
                                          border: renglonErrors[idx] ? '1px solid rgba(239,68,68,0.6)' : '1px solid rgba(96,165,250,0.4)',
                                        }}
                                        placeholder="0"
                                      />
                                      {renglonErrors[idx] && (
                                        <p className="text-xs text-red-400 font-bold mt-1 whitespace-nowrap">{renglonErrors[idx]}</p>
                                      )}
                                    </div>
                                  )}
                                </td>
