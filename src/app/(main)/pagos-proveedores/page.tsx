...
import { usePagosProveedoresStore, nextFacturaConsecutivo, nextPagoConsecutivo, nextAnticipoConsecutivo, type FacturaProveedor, type PagoProveedor, type RenglonPago, type AnticipoProveedor, type AnticipoAplicado } from '@/features/pagos-proveedores/store/pagos-proveedores-store'
...
type EmpresaInfo = { nombre?: string; tipo_identificacion?: string; nro_documento?: string; direccion?: string; ciudad?: string }
type ProvInfo = { tipo_id?: string; nro_documento?: string; direccion?: string; ciudad?: string }

function pdfBaseHTML(titulo: string, badge: string, consecutivo: string, empresaInfo?: EmpresaInfo, empresaLogo: string = LOGO_BASE64) {
  return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>${titulo} ${consecutivo}</title>
  <style>
    @page { size: A4; margin: 1.2cm; }
    * { margin:0; padding:0; box-sizing:border-box; }
    html, body { width:100%; }
    body { font-family:'Segoe UI',Arial,sans-serif; font-size:12px; color:#111; background:#fff; padding:20px; }
    .header { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding-bottom:14px; border-bottom:3px solid #1e3a8a; gap:16px; }
    .header-left { display:flex; align-items:center; gap:14px; flex:1; min-width:0; }
    .logo-box { width:80px; height:80px; flex-shrink:0; border:1px solid #e5e7eb; border-radius:10px; background:#fff; padding:4px; display:flex; align-items:center; justify-content:center; }
    .logo-box img { width:100%; height:100%; object-fit:contain; }
    .company { font-size:18px; font-weight:800; color:#000; line-height:1.2; }
    .company-data { font-size:11px; color:#374151; font-weight:500; margin-top:4px; line-height:1.4; }
    .doc-title { text-align:right; flex-shrink:0; }
    .doc-title h2 { font-size:16px; font-weight:700; color:#1e3a8a; margin-bottom:2px; letter-spacing:.02em; }
    .doc-title .consecutivo { font-size:15px; font-family:monospace; font-weight:900; color:#000; }
    .badge { display:inline-block; padding:3px 10px; border-radius:14px; font-size:10px; font-weight:700; background:#1e3a8a; color:#fff; margin-top:4px; }
    .prov-box { margin-bottom:14px; padding:10px 14px; background:#f0f4ff; border:1px solid #c7d2fe; border-radius:6px; }
    .prov-box h3 { font-size:11px; font-weight:700; color:#1e3a8a; margin-bottom:6px; text-transform:uppercase; letter-spacing:.06em; }
    .prov-box .prov-grid { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:6px 14px; }
    .prov-box .prov-field label { font-size:9px; text-transform:uppercase; letter-spacing:.06em; color:#1e3a8a; font-weight:700; display:block; margin-bottom:1px; }
    .prov-box .prov-field span { font-weight:600; color:#111; font-size:11px; }
    .grid { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin-bottom:16px; padding:12px 14px; background:#eef2ff; border-radius:6px; border:1px solid #c7d2fe; }
    .field label { font-size:9px; text-transform:uppercase; letter-spacing:.06em; color:#1e3a8a; font-weight:700; display:block; margin-bottom:2px; }
    .field span { font-weight:600; color:#111; font-size:11px; }
    ...
    tbody td { padding:6px 10px; font-size:11px; color:#000; font-weight:500; border-bottom:1px solid #e5e7eb; }
    .total-row td { background:#1e3a8a !important; color:#fff !important; font-size:13px; font-weight:800; padding:10px; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    .obs { border:1px solid #1e3a8a; border-radius:6px; overflow:hidden; margin-bottom:14px; }
    .obs-label { font-size:10px; text-transform:uppercase; letter-spacing:.05em; color:#fff; background:#1e3a8a; padding:6px 12px; font-weight:700; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    .obs-content { padding:10px 12px; font-size:11px; color:#1f2937; background:#f8fafc; }
    .footer { margin-top:30px; display:flex; justify-content:space-around; gap:20px; }
    .sign-box { text-align:center; flex:1; }
    .sign-line { border-top:1.5px solid #000; margin:0 auto 4px; padding-top:4px; font-size:10px; font-weight:700; color:#000; }
    @media print { body { padding:0; } .header, table thead tr, .total-row td, .obs-label { -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; } }
  </style></head><body>
  <div class="header">
    <div class="header-left">
      <div class="logo-box"><img src="${empresaLogo}" alt="Logo" /></div>
      <div style="min-width:0;flex:1">
        <div class="company">${empresaInfo?.nombre || 'EMPRESA'}</div>
        ${empresaInfo ? `<div class="company-data">${empresaInfo.tipo_identificacion || ''}${empresaInfo.tipo_identificacion ? ': ' : ''}${empresaInfo.nro_documento || ''}${empresaInfo.direccion ? ` · ${empresaInfo.direccion}` : ''}${empresaInfo.ciudad ? `, ${empresaInfo.ciudad}` : ''}</div>` : ''}
      </div>
    </div>
    <div class="doc-title">
      <h2>${titulo}</h2>
...
function generatePagoPDF(p: PagoProveedor, empresaInfo?: EmpresaInfo, provInfo?: ProvInfo, empresaLogo: string = LOGO_BASE64) {
  const facturasRows = p.facturas_aplicadas.map((rn, i) => `
    <tr style="background:${i % 2 === 0 ? '#f9fafb' : '#fff'}">
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-family:monospace;font-size:12px">${rn.factura_consecutivo}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-family:monospace;font-size:12px">${rn.nro_factura}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb">${rn.concepto || '—'}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace">${fmtMoney(rn.monto_sin_impuesto)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace">${fmtMoney(rn.monto_iva)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace;color:#b91c1c">-${fmtMoney(rn.retencion_fuente + rn.retencion_iva)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace;color:#b91c1c">${rn.anticipo > 0 ? `-${fmtMoney(rn.anticipo)}` : '—'}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace;font-weight:800;color:#1e3a8a">${fmtMoney(rn.monto_aplicado)}</td>
    </tr>`).join('')

  const anticiposRows = (p.anticipos_aplicados || []).map((a, i) => `
    <tr style="background:${i % 2 === 0 ? '#fef3c7' : '#fffbeb'}">
      <td style="padding:8px 12px;border-bottom:1px solid #fde68a;font-family:monospace;font-size:12px">${a.anticipo_consecutivo}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #fde68a;text-align:right;font-family:monospace;color:#92400e;font-weight:700">${fmtMoney(a.monto_aplicado)}</td>
    </tr>`).join('')

  const html = pdfBaseHTML('COMPROBANTE DE PAGO', p.estado, p.consecutivo, empresaInfo, empresaLogo) + `
  <div class="grid">
    <div class="field"><label>Nro. Pago</label><span>${p.consecutivo}</span></div>
    <div class="field"><label>Fecha de Registro</label><span>${fDate(p.fecha_registro)}</span></div>
    <div class="field"><label>Forma de Pago</label><span>${p.forma_pago}</span></div>
    <div class="field"><label>Banco</label><span>${p.banco || '—'}</span></div>
    <div class="field"><label>N° Referencia</label><span>${p.nro_referencia || '—'}</span></div>
    <div class="field"><label>Estado</label><span>${p.estado}</span></div>
  </div>
  ...
  <div class="prov-box">
    ...
    <div class="prov-grid">
      ...
      <div class="prov-field"><label>Dirección</label><span>${provInfo?.direccion || '—'}${provInfo?.ciudad ? `, ${provInfo.ciudad}` : ''}</span></div>
    </div>
  </div>

  <h3 style="font-size:13px;font-weight:700;color:#1e3a8a;margin-bottom:8px;text-transform:uppercase;letter-spacing:.06em;">Facturas Aplicadas</h3>
  <table>
    <thead><tr>
      <th>Consecutivo</th><th>N° Factura</th><th>Concepto</th>
      <th style="text-align:right">Sin Imp.</th>
    ...
    <tbody>${facturasRows}
      <tr class="total-row"><td colspan="7" style="text-align:right">TOTAL DEL PAGO</td><td style="text-align:right;font-family:monospace">${fmtMoney(p.monto_total)}</td></tr>
    </tbody>
  </table>

  ${anticiposRows ? `
  <h3 style="font-size:13px;font-weight:700;color:#92400e;margin-bottom:8px;text-transform:uppercase;letter-spacing:.06em;">Anticipos Aplicados en este Pago</h3>
  <table>
    <thead style="background:#92400e"><tr>
      <th>Consecutivo Anticipo</th><th style="text-align:right">Monto Aplicado</th>
    </tr></thead>
    <tbody>${anticiposRows}
      <tr style="background:#fef3c7"><td style="padding:8px 12px;text-align:right;font-weight:700;color:#92400e">TOTAL ANTICIPOS</td><td style="padding:8px 12px;text-align:right;font-family:monospace;font-weight:800;color:#92400e">${fmtMoney(p.monto_anticipos_aplicados || 0)}</td></tr>
    </tbody>
  </table>
  ` : ''}
  ...
  <div class="footer">
    <div class="sign-box"><div class="sign-line">Elaborado por</div><div style="font-size:11px;font-weight:700;color:#000">_______________</div></div>
    ...
    <div class="sign-box"><div class="sign-line">Recibido por (Proveedor)</div><div style="font-size:11px;font-weight:700;color:#000">_______________</div></div>
  </div>

  <script>
    window.addEventListener('load',()=>{
      const imgs = Array.from(document.images)
      const wait = imgs.map(i => i.complete ? Promise.resolve() : new Promise(res => { i.onload = i.onerror = res }))
      Promise.all(wait).then(()=>setTimeout(()=>window.print(),300))
    })
  <\/script>
  </body></html>`
...
function generateAnticipoPDF(a: AnticipoProveedor, empresaInfo?: EmpresaInfo, provInfo?: ProvInfo, empresaLogo: string = LOGO_BASE64) {
  const html = pdfBaseHTML('COMPROBANTE DE ANTICIPO', a.estado, a.consecutivo, empresaInfo, empresaLogo) + `
  <div class="grid">
    <div class="field"><label>Nro. Anticipo</label><span>${a.consecutivo}</span></div>
    <div class="field"><label>Fecha de Registro</label><span>${fDate(a.fecha_registro)}</span></div>
    <div class="field"><label>Forma de Pago</label><span>${a.forma_pago}</span></div>
    <div class="field"><label>Banco</label><span>${a.banco || '—'}</span></div>
    <div class="field"><label>N° Referencia</label><span>${a.nro_referencia || '—'}</span></div>
    <div class="field"><label>Aprobado por</label><span>${a.persona_aprueba || '—'}</span></div>
  </div>

  <div class="prov-box">
    <h3>Datos del Proveedor</h3>
    <div class="prov-grid">
      <div class="prov-field"><label>Proveedor</label><span>${a.proveedor || '—'}</span></div>
      <div class="prov-field"><label>Tipo ID</label><span>${provInfo?.tipo_id || '—'}</span></div>
      <div class="prov-field"><label>Nro. Documento</label><span>${provInfo?.nro_documento || '—'}</span></div>
      <div class="prov-field"><label>Dirección</label><span>${provInfo?.direccion || '—'}${provInfo?.ciudad ? `, ${provInfo.ciudad}` : ''}</span></div>
    </div>
  ...
  <table>
    ...
    <tbody>
      <tr style="background:#f9fafb">
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb">${a.concepto || '—'}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace;font-size:14px;font-weight:800;color:#1e3a8a">${fmtMoney(a.monto)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;text-align:right;font-family:monospace;color:#92400e;font-weight:700">${fmtMoney(a.saldo_disponible || 0)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;text-align:center">
          <span style="padding:3px 12px;border-radius:12px;font-size:11px;font-weight:700;background:#1e3a8a;color:#fff">${a.estado}</span>
        </td>
      </tr>
      <tr class="total-row"><td colspan="1" style="text-align:right">TOTAL DEL ANTICIPO</td><td colspan="3" style="text-align:right;font-family:monospace">${fmtMoney(a.monto)}</td></tr>
    </tbody>
  </table>

  ${a.observaciones ? `<div class="obs"><div class="obs-label">Observaciones</div><div class="obs-content">${a.observaciones}</div></div>` : ''}

  <div class="footer">
    <div class="sign-box"><div class="sign-line">Elaborado por</div><div style="font-size:11px;font-weight:700;color:#000">_______________</div></div>
    <div class="sign-box"><div class="sign-line">Aprobado por</div><div style="font-size:11px;font-weight:700;color:#000">${a.persona_aprueba || '_______________'}</div></div>
    <div class="sign-box"><div class="sign-line">Recibido por (Proveedor)</div><div style="font-size:11px;font-weight:700;color:#000">_______________</div></div>
  </div>
...
export default function PagosProveedoresPage() {
  ...
  const todosProveedores = useProveedoresStore(s => s.proveedores)
        ...
        <>
          ...
          <div className="rounded-2xl overflow-x-auto" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <table className="w-full text-base text-left">
              <thead style={{ background: 'rgba(255,255,255,0.05)' }}>
                <tr>
                  {['Consecutivo', 'F. Registro', 'Proveedor', 'Forma Pago', 'Banco', 'N° Referencia', 'Facturas', 'Total', 'Estado', 'Acciones'].map(h => (
                    <th key={h} className="px-3 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.6)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {pagos.length === 0 ? (
                  <tr><td colSpan={10} className="px-6 py-12 text-center text-white/30">No hay pagos registrados. Crea el primero con <strong>+ Nuevo Pago</strong>.</td></tr>
                ) : pagos.map(p => (
                  <tr key={p.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }} className="hover:bg-white/[0.02]">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            ...
                            const prov = todosProveedores.find(x => x.nombre === p.proveedor)
                            ...
                            generatePagoPDF(p, empData, provData, emp?.logo || LOGO_BASE64)
                          }}
                          title="Imprimir comprobante"
                          className="px-2 py-1 rounded-lg text-xs font-bold"
                          style={{ background: 'rgba(168,85,247,0.25)', color: '#c4b5fd', border: '1px solid rgba(168,85,247,0.5)' }}
                        >📄 PDF</button>
                        <span className="font-mono font-bold text-blue-300">{p.consecutivo}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <button onClick={() => {
                        ...
                        const provData = prov ? { tipo_id: prov.tipo_id, nro_documento: prov.nro_documento, direccion: prov.direccion, ciudad: prov.ciudad } : undefined
                        generatePagoPDF(p, empData, provData, emp?.logo || LOGO_BASE64)
                      }} className="px-2 py-1 rounded-lg text-xs font-medium" style={{ background: 'rgba(168,85,247,0.2)', color: '#c4b5fd', border: '1px solid rgba(168,85,247,0.3)' }}>📄 PDF</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <>
          ...
          <div className="rounded-2xl overflow-x-auto" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <table className="w-full text-base text-left">
              <thead style={{ background: 'rgba(255,255,255,0.05)' }}>
                <tr>
                  {['Consecutivo', 'F. Registro', 'Proveedor', 'Forma Pago', 'Banco', 'Referencia', 'Aprobado por', 'Concepto', 'Monto', 'Aplicación', 'Estado', 'Acciones'].map(h => (
                    <th key={h} className="px-3 py-3 text-xs font-semibold uppercase tracking-wider whitespace-nowrap" style={{ color: 'rgba(255,255,255,0.6)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {anticipos.length === 0 ? (
                  ...
                  <tr key={a.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }} className="hover:bg-white/[0.02]">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
                            const emp = empresas[0]
                            const empData = emp ? { nombre: emp.nombre, tipo_identificacion: emp.tipo_identificacion, nro_documento: emp.nro_documento, direccion: emp.direccion, ciudad: emp.ciudad } : undefined
                            const prov = todosProveedores.find(x => x.nombre === a.proveedor)
                            const provData = prov ? { tipo_id: prov.tipo_id, nro_documento: prov.nro_documento, direccion: prov.direccion, ciudad: prov.ciudad } : undefined
                            generateAnticipoPDF(a, empData, provData, emp?.logo || LOGO_BASE64)
                          }}
                          title="Imprimir comprobante"
                          className="px-2 py-1 rounded-lg text-xs font-bold"
                          style={{ background: 'rgba(168,85,247,0.25)', color: '#c4b5fd', border: '1px solid rgba(168,85,247,0.5)' }}
                        >📄 PDF</button>
                        <span className="font-mono font-bold text-blue-300">{a.consecutivo}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      {(() => {
                        const saldo = a.saldo_disponible || 0
                        const monto = a.monto || 0
                        if (saldo >= monto && a.estado !== 'Anulado') {
          ...
          <div className="w-full max-w-2xl rounded-2xl p-6 max-h-[90vh] overflow-y-auto" style={{ background: 'rgba(15,23,42,0.97)', border: '1px solid rgba(255,255,255,0.15)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <PersonalSelector
                  value={antPersonaAprueba}
                  onChange={setAntPersonaAprueba}
                  placeholder="Seleccione el autorizante…"
                />
              </div>
              <div className="md:col-span-2">
