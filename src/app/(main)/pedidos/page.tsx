...
export default function PedidosPage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const pedidoToSave = selectedPedido.fecha_aprobacion
      ...
      : selectedPedido
    if (pedidoToSave.id) {
      usePedidosStore.setState({ pedidos: pedidos.map(p => p.id === pedidoToSave.id ? pedidoToSave : p) })
    } else {
      usePedidosStore.setState({ pedidos: [...pedidos, { ...pedidoToSave, id: crypto.randomUUID() }] })
    }
    setIsFormOpen(false)
              ...
              <div className="mb-8 rounded-2xl p-6" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }}>
                ...
                <form onSubmit={handleSave}>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    ] as { label: string; key: string; type: string; placeholder?: string }[]).map(({ label, key, type = 'text', placeholder }) => (
                      <div key={key}>
                    ...
                    {/* Comprador — selector de Personal Empresa */}
                    <div>
                      <label className="block text-xl text-white font-extrabold mb-1">Comprador</label>
                      <PersonalSelector
                        value={selectedPedido.comprador}
                        onChange={v => setSelectedPedido({ ...selectedPedido, comprador: v })}
                    ...
                    <tr key={p.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex gap-1.5 whitespace-nowrap">
                          <button onClick={() => setViewDetail(p)} className="px-2.5 py-1 rounded-lg text-xs font-medium" style={{ background: 'rgba(59, 130, 246, 1)', color: '#fff', border: '1px solid rgba(59, 130, 246, 0.6)' }}>Ver</button>
                          ...
                          {permisos.esAdmin && <button onClick={() => handleDelete(p)} className="px-2.5 py-1 rounded-lg text-xs font-medium" style={{ background: 'rgba(239, 68, 68, 1)', color: '#fff', border: '1px solid rgba(239, 68, 68, 0.6)' }}>{tBtn('delete')}</button>}
                        </div>
                      </td>
                    </tr>
                  )
